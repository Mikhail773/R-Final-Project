...
Vehicles are essential to transportation as a society and one of the main methods of obtaining a vehicle is through the purchase of a previously owned vehicle. As is the case with many nations worldwide Belarus has an extensive used vehicle market and the dataset in question is of used cars in a popular catalog from the country of Belarus. This dataset will be cleaned and analyzed to answer several questions and accomplish our main two goals. Multiple Linear Regression, SVR, Decision Tree Regression, Random Forest Regression, and KNN will all be used to predict the market price of a used car in Belarus given the attributes found in the dataset. Also, a Decision Tree and a Logistic Regression Model were used to create an exploratory and predictive model for the exchangeability of a vehicle in Belarus. The accuracy of these models was analyzed, and performance tested for real-world application with the help of a training and testing dataset.

**OUR FINDINGS:** What we have found is that the Random forest tree had the best R2 value and error rate, meaning it had the best predictions. The two worst prediction models were the Multiple Linear Regression, and SVR Radial. Here is the list of all our results from each of our models. The lower the error rate the better, and the higher the R2 value the better the correlation the model has with the data.
...
The AUC, accuracy, sensitivity, and specificity are higher in the decision tree. All these values allow us to conclude that the Decision Tree produces a better predictive model.

# Introduction:

## Focus Problems

There are several objectives that will be the focal point of the research done on the used cars dataset. The main scope of this project is to create an effective predictive model for the price of a used in Vehicle in Belarus. As well as creating a predictive model for vehicle prices an exploratory and predictive model will be created for the likelihood of a vehicle owner being willing to exchange their vehicle. Several questions will be answered in the process of creating models and to gain a further understanding of the overall dataset.
...
Once the unnecessary attributes were filtered and the NAs were accounted for it important to investigate for duplicate samples. The code for doing this procedure was as follows:
...
# The graph seems to almost be normally distributed minus what appears to be some outliers on the older end of the years
```
...
To answer this question, we decided to use a bar chart to see the average prices per region in comparison with each other.
...
Two bar graphs were used in solving this question. First a bar graph was used to plot the distribution of vehicles for each manufacturer. Then another bar graph was used as a means of quickly inspecting how average price ranged between different manufacturers. As useful as these graphs were in understanding the relationship between price and manufacturer it proved to be insufficient in proving a result. To prove that there existed a significant relationship between manufacturer and asking price of a car One-Way Anova was utilized. Once again we were dealing with a categorical data with several categories which lends the problem nicely to this sort of test.
...
To create a good exploratory model for exchangeability we will deploy a Decision tree. The model will be running the train function with 10-fold cross-validation and a tune-length of 10(number of cp values to evaluate) l. These settings should prune our tree and ensured an optimal decision tree.
...
We attempted to create a more accurate exploratory model for exchangeability by using Logistic Classification. The model will be running the train function with 10-fold cross-validation and a tune-length of 10(number of cp values to evaluate) l. These settings should optimally model.
...
The VIF tells us that there is not significant multi-collinearity between any of the categorical attributes.
